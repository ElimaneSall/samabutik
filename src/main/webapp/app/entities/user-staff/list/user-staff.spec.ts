...
import { sampleWithRequiredData } from '../user-staff.test-samples';

import { UserStaff } from './user-staff';

vitest.useFakeTimers();

describe('UserStaff Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 5345 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.userStaffs()[0]).toEqual(expect.objectContaining({ id: 5345 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 5345 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.userStaffs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 5345 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to userStaffService', () => {
      ...
      vitest.spyOn(service, 'getUserStaffIdentifier');
      const id = comp.trackId(entity);
      expect(service.getUserStaffIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.userStaffsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
