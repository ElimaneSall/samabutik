...
import { sampleWithRequiredData } from '../pack-item.test-samples';
...
describe('PackItem Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PackItem;
  let fixture: ComponentFixture<PackItem>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PackItem);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4307 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.packItems()[0]).toEqual(expect.objectContaining({ id: 4307 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4307 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.packItems()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4307 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to packItemService', () => {
      const entity = { id: 4307 };
      vitest.spyOn(service, 'getPackItemIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPackItemIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.packItemsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
