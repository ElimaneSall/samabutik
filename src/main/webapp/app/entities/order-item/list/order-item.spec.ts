...
describe('OrderItem Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 25971 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 25971 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.orderItems()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 25971 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to orderItemService', () => {
      ...
      vitest.spyOn(service, 'getOrderItemIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.orderItemsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
